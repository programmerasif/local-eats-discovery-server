...
var uri =
  "mongodb://local-eats-discovery:vuQeazZOIiF2EOdi@ac-wotlaa2-shard-00-00.0rmdzda.mongodb.net:27017,ac-wotlaa2-shard-00-01.0rmdzda.mongodb.net:27017,ac-wotlaa2-shard-00-02.0rmdzda.mongodb.net:27017/?ssl=true&replicaSet=atlas-as340s-shard-0&authSource=admin&retryWrites=true&w=majority";
...
async function run() {
  try {
    ...
    const foodItems = client
      .db("local-eats-discovery-server")
      .collection("items");
    ...
    app.patch("/user-update/:id", async (req, res) => {
      try {
        const id = req.params.id;
        const { name, email, phNumber, place_name, displayPhoto, location } =
          req.body;
    ...
    app.get("/verify-user/:email", async (req, res) => {
      try {
        ...
        console.error("Error identifying the role:", error);
        res.status(500).send("Internal Server Error");
      }
    });
